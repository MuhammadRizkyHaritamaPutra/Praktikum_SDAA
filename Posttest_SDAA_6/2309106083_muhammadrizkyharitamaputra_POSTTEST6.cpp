...
int fibonacciSearch(vector<SepedaMotor>& vec, double x) {
    int n = vec.size();
    int fibMMm2 = 0;  
    int fibMMm1 = 1;  
    ...
    if (fibMMm1 && vec[offset + 1].harga == x) {
        return offset + 1;
    }
...
int jumpSearch(vector<SepedaMotor>& vec, double x) {
    int n = vec.size();
    int step = (int)sqrt(n);
    int prev = 0;

    while (vec[min(step, n) - 1].harga < x) {
        prev = step;
        step += sqrt(n);
        if (prev >= n) {
            return -1;
...
void tampilkanMenuMotor() {
    ...
    cout << "10. Cari Sepeda Motor Berdasarkan Harga (Fibonacci Search)" << endl;
    cout << "11. Cari Sepeda Motor Berdasarkan Harga (Jump Search)" << endl;
    cout << "12. Cari Sepeda Motor Berdasarkan Nama (Boyer-Moore Search)" << endl;
    cout << "13. Keluar" << endl;
}
                        ...
                        switch (pilihanMotor) {
                           ...
                           case 7:
                            ...
                            case 10: {
                                ...
                                cin >> harga;
                                vector<SepedaMotor> vec = linkedListToVector(daftarMotor);
                                int indeks = fibonacciSearch(vec, harga);
                                if (indeks != -1) {
                                    cout << "Sepeda motor ditemukan pada indeks " << indeks + 1 << " dengan harga " << vec[indeks].harga << endl;
                                } else {
                                    cout << "Sepeda motor tidak ditemukan." << endl;
                                }
                                break;
                            }
                            case 11: {
                                ...
                                cin >> harga;
                                vector<SepedaMotor> vec = linkedListToVector(daftarMotor);
                                int indeks = jumpSearch(vec, harga);
                                if (indeks != -1) {
                                    cout << "Sepeda motor ditemukan pada indeks " << indeks + 1 << " dengan harga " << vec[indeks].harga << endl;
                                } else {
                                    cout << "Sepeda motor tidak ditemukan." << endl;
                            ...
                            case 12: {
                                system("cls");
                                string nama;
                                cout << "Masukkan nama sepeda motor yang dicari: ";
                                cin.ignore();
                                getline(cin, nama);
                                Node* temp = daftarMotor;
                                while (temp != nullptr) {
                                    if (boyerMooreSearch(temp->motor.nama, nama) != -1) {
                                        cout << "Sepeda motor " << temp->motor.nama << " ditemukan." << endl;
                                        break;
                                    }
                                    temp = temp->next;
                                }
